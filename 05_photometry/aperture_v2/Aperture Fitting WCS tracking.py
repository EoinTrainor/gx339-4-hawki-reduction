# title: GX 339-4 Robust Aperture Fitting (WCS Tracked Comp Stars + Locked Annulus + Core Npix SNR + Max SNR Comp Star + Locked GX Aperture + Background Structure + Mask Overlays + Screening + Per Frame Summary)
...
# Comparison stars WCS coordinates (ICRS, degrees) from your reference frame centroids
STAR_WCS_DEG = {
...
# Stability criterion (kept for diagnostics; final selection is locked-to-comp as requested)
STAB_CONSECUTIVE = 2
STAB_SIGMA = 2.0
...
date_obs = header.get("DATE-OBS", "UNKNOWN_DATE")
...
# ---------------- STAGE 1: PSF SCALE FROM COMPARISON STARS (WCS TRACKED) ----------------
star_results = []
fwhm_list = []

for name, (ra_deg, dec_deg) in STAR_WCS_DEG.items():
    # WCS seeded pixel guess for this frame
    x_guess, y_guess = wcs_to_pixel(wcs, ra_deg, dec_deg)

    # local centroid refinement
    ...
    # peak pixel in cutout for saturation screening
...
if not np.isfinite(fwhm_med):
    raise RuntimeError("FWHM estimation failed for comparison stars. Check WCS, coords, or GAUSS_FIT_HALF_SIZE.")
...
# ---------------- COMP STAR MEASUREMENT AT CHOSEN APERTURE ----------------
...
# ---------------- STAGE 3: APPLY TO GX AND CHOOSE GX R ----------------
...
# --- GX peak SNR (for reporting % of max at chosen radius) ---
gx_max_snr = np.nan
...
# (diagnostic only) run stability chooser to report range
gx_r_stab, gx_idx_stab = choose_gx_radius_by_stability(
    gx_r, gx_net, gx_err, consecutive=STAB_CONSECUTIVE, sigma=STAB_SIGMA
)

# ---------------- GX APERTURE: LOCK TO BEST COMPARISON STAR (PEAK SNR) ----------------
...
# ---------------- GX RELATIVE BRIGHTNESS (RATIO TO COMP STAR) ----------------
...
print("Note: multiply this ratio by the comparison star calibrated flux later.")
print("=========================================================\n")
...
# (3) Stability diagnostic: |ΔF| vs allowed (diagnostic only)
...
if r2.size >= 3:
    ...
    ax.set_title("GX stability test: flux change vs allowed change (diagnostic)")
...
# (5) Background structure view (inferno) + black aperture circle
...
if WRITE_SUMMARY_CSV:
    ...
    row = {
        ...
        "best_comp_r_ap_used_px": comp_r_ap_out,
