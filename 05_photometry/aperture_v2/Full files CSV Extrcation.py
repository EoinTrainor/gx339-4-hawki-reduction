...
Adds (requested):
Printed per FITS (in console_log.txt):
  - For Star 3:
      full flux (raw aperture sum) + error
      background in aperture + error
      net flux + error
      chosen radius (max SNR radius)
  - For GX 339-4:
      full flux + error
      background in aperture + error
      net flux + error
      chosen radius (locked to Star 3)
      SNR at chosen radius
      max SNR and radius of max SNR

Also writes two master CSVs across directory:
  - OUT_ROOT/gx_all_frames.csv
  - OUT_ROOT/star3_all_frames.csv

Error model note:
  We keep your SNR3 variance:
    var(net) = sigma_sky^2 * (A + A^2 / N_core)
  We split that into:
    raw_err      = sigma_sky * sqrt(A)
    bg_err       = sigma_sky * (A / sqrt(N_core))
    net_err      = sqrt(raw_err^2 + bg_err^2)
  This matches your existing net_err.

(Still does plot saving OB01–OB08 and summary_all_frames.csv as before.)
"""
...
# New: two master CSVs (requested)
GX_ALL_FRAMES_CSV = os.path.join(OUT_ROOT, "gx_all_frames.csv")
STAR3_ALL_FRAMES_CSV = os.path.join(OUT_ROOT, "star3_all_frames.csv")
...
def flux_components_from_background(raw_sum, ap_area, B, sigma_sky, n_core):
    """
    Returns:
      full_flux (raw_sum), full_err,
      bg_flux (B*A),      bg_err,
      net_flux,           net_err,
      snr3
    """
    ...
    if (A <= 0) or (sigma_sky <= 0) or (n_core is None) or (n_core <= 0):
        return (raw, np.nan, bg, np.nan, net, np.nan, np.nan)

    full_err = float(sigma_sky * np.sqrt(A))
    bg_err = float(sigma_sky * (A / np.sqrt(float(n_core))))
    net_err = float(np.sqrt(full_err**2 + bg_err**2))
    snr3 = float(net / net_err) if (np.isfinite(net_err) and net_err > 0) else np.nan

    return (raw, full_err, bg, bg_err, net, net_err, snr3)
...
def process_one_frame(fits_path: str, outdir: str):
    ...
    print(f"{'Star':<7} {'FWHM(px)':>9} {'maxSNR3':>10} {'opt r(px)':>10} {'sens':>8} {'coreN':>8} {'sat':>5} {'fit':>5} {'score':>12}")
    for r in ranking_rows:
        print(
            f"{r['name']:<7} {r['fwhm']:>9.2f} {r['max_snr']:>10.3f} {r['r_opt']:>10.2f} {r['sens']:>8.3f} {r['coreN']:>8d} "
            f"{int(r['sat_flag']):>5d} {int(r['fit_flag']):>5d} {r['score']:>12.2f}"
        )
    print("==========================================================================\n")

    # -------- Star 3 measurement at its chosen radius (max SNR) --------
    # (In your current setup, Star 3 is the only comp star, so best == Star 3.)
    comp_name = best["name"]
    comp_meta = best["meta"]
    comp_sweep = best["sweep"]

    comp_r = comp_sweep[:, 0]
    comp_rawsum = comp_sweep[:, 1]
    comp_area = comp_sweep[:, 2]
    comp_net = comp_sweep[:, 3]
    comp_neterr = comp_sweep[:, 4]
    comp_snr = comp_sweep[:, 5]

    comp_idx = None
    if np.isfinite(best.get("r_opt", np.nan)):
        comp_idx = int(np.argmin(np.abs(comp_r - best["r_opt"])))

    # Outputs (raw/bg/net components)
    comp_r_ap_out = np.nan
    comp_full_out = np.nan
    comp_full_err_out = np.nan
    comp_bg_out = np.nan
    comp_bg_err_out = np.nan
    comp_net_out = np.nan
    comp_net_err_out = np.nan
    comp_snr_out = np.nan

    if comp_idx is not None:
        comp_r_ap_out = float(comp_r[comp_idx])
        raw = float(comp_rawsum[comp_idx])
        area = float(comp_area[comp_idx])

        (full_flux, full_err,
         bg_flux, bg_err,
         net_flux, net_err,
         snr3) = flux_components_from_background(
            raw, area,
            comp_meta["B"],
            comp_meta["sigma_sky"],
            comp_meta["n_core"]
        ...
        comp_full_out = full_flux
        comp_full_err_out = full_err
        comp_bg_out = bg_flux
        comp_bg_err_out = bg_err
        comp_net_out = net_flux
        comp_net_err_out = net_err
        comp_snr_out = snr3

        print("================= STAR 3 (COMP) APERTURE PHOTOMETRY =================")
        print(f"DATE-OBS                 = {date_obs}")
        print(f"Star                     = {comp_name}")
        print(f"Chosen radius (max SNR)   = {comp_r_ap_out:.3f} px")
        print("")
        print(f"Full flux (raw sum)       = {comp_full_out:.3f} ± {comp_full_err_out:.3f} ADU")
        print(f"Background in aperture    = {comp_bg_out:.3f} ± {comp_bg_err_out:.3f} ADU")
        print(f"Net flux                  = {comp_net_out:.3f} ± {comp_net_err_out:.3f} ADU")
        print(f"SNR3 at chosen radius     = {comp_snr_out:.3f}")
        print("======================================================================\n")
    ...
    gx_r = gx_sweep[:, 0]
    gx_rawsum = gx_sweep[:, 1]
    gx_area = gx_sweep[:, 2]
    gx_net = gx_sweep[:, 3]
    gx_neterr = gx_sweep[:, 4]
    gx_snr = gx_sweep[:, 5]

    # GX peak SNR (diagnostic)
    gx_max_snr = np.nan
    ...
    # lock GX radius to comp r_opt (requested)
    ...
    # Outputs for GX at chosen radius
    gx_r_ap_out = np.nan
    gx_full_out = np.nan
    gx_full_err_out = np.nan
    gx_bg_out = np.nan
    gx_bg_err_out = np.nan
    gx_net_out = np.nan
    gx_net_err_out = np.nan
    gx_snr_out = np.nan

    if gx_idx_choice is not None:
        gx_r_ap_out = float(gx_r[gx_idx_choice])
        raw = float(gx_rawsum[gx_idx_choice])
        area = float(gx_area[gx_idx_choice])
        ...
        gx_full_out = full_flux
        gx_full_err_out = full_err
        gx_bg_out = bg_flux
        gx_bg_err_out = bg_err
        gx_net_out = net_flux
        gx_net_err_out = net_err
        gx_snr_out = snr3

        print("================= GX 339-4 APERTURE PHOTOMETRY =================")
        print(f"DATE-OBS                     = {date_obs}")
        print(f"Chosen radius (locked to Star 3 max SNR) = {gx_r_ap_out:.3f} px")
        print("")
        print(f"Full flux (raw sum)           = {gx_full_out:.3f} ± {gx_full_err_out:.3f} ADU")
        print(f"Background in aperture        = {gx_bg_out:.3f} ± {gx_bg_err_out:.3f} ADU")
        print(f"Net flux                      = {gx_net_out:.3f} ± {gx_net_err_out:.3f} ADU")
        print(f"SNR3 at chosen radius         = {gx_snr_out:.3f}")
        print("")
        print(f"Max SNR3 over GX sweep         = {gx_max_snr:.3f} at r = {gx_r_at_max_snr:.3f} px")
        print("================================================================\n")

    # Relative brightness ratio (propagated)
    gx_rel_brightness = np.nan
    gx_rel_err = np.nan
    if np.isfinite(gx_net_out) and np.isfinite(comp_net_out) and comp_net_out != 0:
        gx_rel_brightness = gx_net_out / comp_net_out
        if gx_net_out != 0 and np.isfinite(gx_net_err_out) and np.isfinite(comp_net_err_out):
            gx_rel_err = abs(gx_rel_brightness) * np.sqrt(
                (gx_net_err_out / gx_net_out) ** 2 + (comp_net_err_out / comp_net_out) ** 2
            )

    print("================= GX RELATIVE BRIGHTNESS =================")
    print(f"GX / {comp_name} (net/net) = {gx_rel_brightness:.6f}")
    ...
    ax.plot(comp_sweep[:, 0], comp_sweep[:, 5], marker="o", ms=3)
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.2))
    ax.errorbar(gx_r, gx_net, yerr=gx_neterr, fmt="o", ms=3, capsize=2)
    if np.isfinite(gx_r_choice):
    ...
    ok = np.isfinite(gx_r) & np.isfinite(gx_net) & np.isfinite(gx_neterr) & (gx_neterr > 0)
    r2 = gx_r[ok]
    F2 = gx_net[ok]
    E2 = gx_neterr[ok]
    if r2.size >= 3:
    ...
    # OB07: 2x2 diagnostics panel (unchanged)
    ...
    # -------- Stage 5: write summary row (master CSV as before) --------
    if WRITE_SUMMARY_CSV:
        row = {
            ...
            "comp_net_flux_adu": comp_net_out,
            "comp_flux_err_adu": comp_net_err_out,
            "gx_x_px": gx_x,
            "gx_y_px": gx_y,
            "gx_r_choice_px": gx_r_choice,
            "gx_r_ap_used_px": gx_r_ap_out,
            "gx_net_flux_adu": gx_net_out,
            "gx_flux_err_adu": gx_net_err_out,
            "gx_snr3": gx_snr_out,
        ...
        with open(SUMMARY_ALL_FRAMES, "a", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=list(row.keys()))
            if not file_exists:
                w.writeheader()
            w.writerow(row)

    # -------- Stage 6: write requested TWO CSVs (GX and Star 3) --------
    # Star 3 CSV row
    star3_row = {
        "fits_file": fits_base,
        "date_obs": date_obs,
        ...
        "ann_in_px": ann_in,
        "ann_out_px": ann_out,
        "B_adu_per_pix": comp_meta["B"],
        "sigma_sky_adu_per_pix": comp_meta["sigma_sky"],
        "ann_coreN": comp_meta["n_core"],
        "r_chosen_px": comp_r_ap_out,
        "full_flux_adu": comp_full_out,
        "full_flux_err_adu": comp_full_err_out,
        "bg_flux_adu": comp_bg_out,
        "bg_flux_err_adu": comp_bg_err_out,
        "net_flux_adu": comp_net_out,
        "net_flux_err_adu": comp_net_err_out,
        "snr3_at_r": comp_snr_out,
        "max_snr3": float(np.nanmax(comp_snr)) if np.isfinite(comp_snr).any() else np.nan,
        "r_at_max_snr3_px": float(comp_r[int(np.nanargmax(comp_snr))]) if np.isfinite(comp_snr).any() else np.nan,
    }
    append_row_csv(STAR3_ALL_FRAMES_CSV, star3_row)
    ...
    gx_row = {
        ...
        "ann_in_px": ann_in,
        "ann_out_px": ann_out,
        "B_adu_per_pix": gx_meta["B"],
        "sigma_sky_adu_per_pix": gx_meta["sigma_sky"],
        "ann_coreN": gx_meta["n_core"],
        "r_chosen_px": gx_r_ap_out,
        "full_flux_adu": gx_full_out,
        "full_flux_err_adu": gx_full_err_out,
        "bg_flux_adu": gx_bg_out,
        "bg_flux_err_adu": gx_bg_err_out,
        "net_flux_adu": gx_net_out,
        "net_flux_err_adu": gx_net_err_out,
        "snr3_at_r": gx_snr_out,
        "max_snr3": gx_max_snr,
        "r_at_max_snr3_px": gx_r_at_max_snr,
        "gx_over_star3_net_ratio": gx_rel_brightness,
        "gx_over_star3_net_ratio_err": gx_rel_err,
        "gx_r_locked_to_star3_px": gx_r_choice,
    }
    append_row_csv(GX_ALL_FRAMES_CSV, gx_row)
...
def main():
    ...
    print("Master GX CSV  :", GX_ALL_FRAMES_CSV)
    print("Master Star3 CSV:", STAR3_ALL_FRAMES_CSV)
    print("------------------------------------------------------------")
